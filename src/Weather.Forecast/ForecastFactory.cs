...
{
    private readonly DateOnly _dateFrom = DateOnly.FromDateTime(TimeProvider.System.GetUtcNow().Date.Add(-TimeSpan.FromDays(DateRange)));
    ...
    public IEnumerable<WeatherForecast> Create(int number = 1)
    {
        ...
        {
            var celsius = TemperatureFrom + (decimal)(Random.Shared.NextDouble() * (double)(TemperatureTo - TemperatureFrom));   
